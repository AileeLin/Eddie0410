...
<%@ page import="com.qa_reply.model.*"%>
...
boolean login_state = false;   
 ...
 if(login_state!=true){
 session.setAttribute("location", "/CA102G4/front_end/question/question.jsp");
...
<%
	...
	if(question_id != null && !"".equals(question_id) ){
		QuestionVO questionVO = new QuestionService().getOneQuestion(question_id);
		pageContext.setAttribute("questionVO", questionVO);
	}else if(question_id ==null || "".equals(question_id)){
		question_id = (String)request.getAttribute("question_id");
	}
	
	Qa_replyService qa_replySvc = new Qa_replyService();
	List<Qa_replyVO> list =qa_replySvc.getQa_replyByQuestion_id(question_id);
...
<head>
	...
	<link href="<%=request.getContextPath()%>/front_end/css/question/question.css" rel="stylesheet" type="text/css" media="all">
...
<body>
	...
	<ol class="breadcrumb">
		...
		<li class="active"> 回覆區</li>
	</ol>
	<div class="container">
		<div class="jumbotron">
			<h4>${questionVO.question_content}</h4>
			...
			<br>
			  ...
			  <!-- 收藏button -->
			  <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#myModal"><i class="collection far fa-heart"></i>收藏</button>
			  ...
			  <div class="modal fade" id="myModal" role="dialog">
			    <div class="modal-dialog">
			      ...
			      <div class="modal-content">
			        ...
			        <div class="modal-footer">
			          	<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/qa_collect.do" name="form1" >
			  ...
			  <!-- 檢舉button -->
			  <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#Modal"><i class="fas fa-ban"></i>檢舉</button>
    ...
    <div class="container">
    	<i class="fa fa-comment">我來回覆</i>
    </div>
    
    <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/qa_reply.do" name="form1"  >
    	<div class="container">	
	...
	<c:forEach var="qa_replyVO" items="${list}"	>
	<div class="container">
		<div class="media">
			<div class="media-left">
				<img src="<%= request.getContextPath()%>/QuestionGetPicture?mem_id=${qa_replyVO.mem_id}" class="media-object" style="width:60px">
			</div>
			<div class="media-body">
				<a class="media-heading">${memberSvc.findByPrimaryKey(qa_replyVO.mem_id).mem_Name}</a> 
				
				<span class="addtime">${qa_replyVO.reply_date}</span>
			
			  <!-- 檢舉button -->
			  <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#Modal1-${qa_replyVO.reply_id}"><i class="fas fa-ban"></i>檢舉</button>
			
			  <!-- Modal -->
			  <div class="modal fade" id="Modal1-${qa_replyVO.reply_id}" role="dialog">
			    <div class="modal-dialog">
			      ...
			      <div class="modal-content">
			        ...
			        <div class="modal-footer">
		 			<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/rp_report.do" name="form1"  >
					 	 <input type="hidden" name="action" value="insert">
					 	  <input type="hidden" name="question_id" value="${questionVO.question_id}">
					 	 <input type="hidden" name="reply_id" value="${qa_replyVO.reply_id}">
	          	 		 <input type="hidden" name="mem_Id" value="${memberVO.mem_Id}">
						<button type="submit" class="btn btn-info" ><i class="fa fa-check"></i>確定</button>
					</FORM>
			        </div>
