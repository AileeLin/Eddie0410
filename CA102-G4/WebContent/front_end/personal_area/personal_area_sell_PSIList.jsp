...
<head>
   ...
   <!-- 登入才會有的功能(賣家取消訂單or確認出貨通知)-->
	<c:if test="${memberVO != null}">
		<script>
		
			function cancelSellOrdMessage(seller_id,buyer_id,order_id){

				var jsonObj = {
					"title"		:"訂單取消",
				  	"sender"	: seller_id,
				 	"receiver"	: buyer_id,
				 	"message"	:"您的訂單編號:"+order_id+"已被賣家取消",
				};
			...
			function confirmShipMessage(seller_id,buyer_id,order_id,ship_id){
				
				var jsonObj = {
					"title"		:"訂單已出貨",
				  	"sender"	: seller_id,
				 	"receiver"	: buyer_id,
				 	"message"	:"您的訂單編號:"+order_id+"已出貨，寄件編號:"+ship_id
				};
				webSocket.send(JSON.stringify(jsonObj));
											...
											<div class="tab-pane active" id="tab_1">
												...
												<div id="ord_block_${ordVO.order_id}">
													...
													<p class="form-submit p-b-20">
														<button type="button" class="btn cancel-order" data-buyerid ="${ordVO.buyer_mem_id}" data-id="${ordVO.order_id}" data-toggle="modal" data-target="#cancelOrderModal">取消訂單</button> 
														<button type="button" class="btn ship-order" id="ship-order-${ordVO.order_id}" data-toggle="modal" data-target="#shipOrderModal" onclick="confirmShip('${ordVO.order_id}','${ordVO.buyer_mem_id}')">確認出貨</button>
													</p>
  ...
  <div class="modal fade" id="shipOrderModal" role="dialog">
    <div class="modal-dialog modal-lg">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
       	 <input type="hidden" name="shipBuyerId" id="shipBuyerId" value=""/>
...
$(document).on("click", ".cancel-order", function () {
    ...
    $(".modal-footer #buyerId").val( buyerId );
   
});

function cancelOrderById(){
	var ordId =  $(".modal-footer #ordId").val();
	var buyerId =  $(".modal-footer #buyerId").val();
	...
	$.ajax({
		...
		success:function(data){
			console.log("賣家取消訂單成功");
			cancelSellOrdMessage("${memId}",buyerId,ordId);
			$('#ord_block_'+ordId).remove();
			window.location.replace("${pageContext.request.contextPath}/front_end/personal_area/personal_area_sell_CLList.jsp");
		}
	})
...
function confirmShip(ordId,buyerId){
	$(".modal-footer #shipOrdId").val( ordId );
	$(".modal-footer #shipBuyerId").val( buyerId );
}

function shipById(){
	var ordId = $(".modal-footer #shipOrdId").val();
	var buyerId = $(".modal-footer #shipBuyerId").val();	
	...
	$.ajax({
		...
		success:function(data){
			console.log("賣家確認出貨");
			confirmShipMessage("${memId}",buyerId,ordId,shipId);
			$('#ord_block_'+ordId).remove();
			window.location.replace("${pageContext.request.contextPath}/front_end/personal_area/personal_area_sell_PRList.jsp");
		}
	})
