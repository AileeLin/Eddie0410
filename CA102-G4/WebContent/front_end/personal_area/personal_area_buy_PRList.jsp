...
<head>
   ...
   <style>
	...
	<c:if test="${memberVO != null}">
		<script>
			function confirmDeliveryMessage(buyer_id,seller_id,order_id,ship_id){
				
				var jsonObj = {
					"title"		:"訂單確認收貨",
				  	"sender"	: buyer_id,
				 	"receiver"	: seller_id,
				 	"message"	:"您的訂單編號:"+order_id+"已被買家確認收貨，款項很快會進行撥款給您"
				};
											...
											<div id="confirm_block_${ordVO.order_id}">
												...
												<c:forEach var="ordDetailsVO" items="${ordDetailsSvc.getOrderDetialsByOrdId(ordVO.order_id)}">	
													...
													<p class="form-submit p-b-20">  
														<button type="button" class="btn confirm-delivery" id="confirm-delivery-${ordVO.order_id}" data-toggle="modal" data-target="#confirmDeliveryModal" onclick="confirmDelivery('${ordVO.order_id}','${ordVO.seller_mem_id}')">確認收貨</button>
													</p>
  ...
  <div class="modal fade" id="confirmDeliveryModal" role="dialog">
    <div class="modal-dialog modal-lg">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <input type="hidden" name="confirmSellerId" id="confirmSellerId" value=""/>
...
function confirmDelivery(ordId,sellerId){
	$(".modal-footer #confirmOrdId").val( ordId );
	$(".modal-footer #confirmSellerId").val( sellerId );
}

function confirmDeliveryById(){
	
	var ordId = $(".modal-footer #confirmOrdId").val();
	var sellerId = $(".modal-footer #confirmSellerId").val();
	...
	$.ajax({
		...
		success:function(data){
			console.log("買家確認收貨");
			confirmDeliveryMessage("${memId}",sellerId,ordId);
			$('#confirm_block_'+ordId).remove();
			window.location.replace("${pageContext.request.contextPath}/front_end/personal_area/personal_area_buy_COMList.jsp");
		}
	})
