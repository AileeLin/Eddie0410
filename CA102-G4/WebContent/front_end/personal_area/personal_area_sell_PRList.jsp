...
<head>
   ...
   <style>
	...
	<c:if test="${memberVO != null}">
		<script>
			function confirmDeliveryMessage(seller_id,buyer_id,order_id,ship_id){
				
				var jsonObj = {
					"title"		:"訂單已到貨",
				  	"sender"	: seller_id,
				 	"receiver"	: buyer_id,
				 	"message"	:"您的訂單編號:"+order_id+"已到貨，寄件編號:"+ship_id+"，請在7天內至門市取件，謝謝"
				};
										...
										<!-- 待收貨  tab2 -->	
											...
											<div class="host">
												...
												<p class="form-submit p-b-20">  
													<button type="button" class="btn confirm-delivery" id="confirm-delivery" data-toggle="modal" data-target="#confirmDeliveryModal" onclick="confirmDelivery('${ordVO.order_id}','${ordVO.buyer_mem_id}','${ordVO.shipment_id}')">確認到貨</button>
												</p>
  ...
  <div class="modal fade" id="confirmDeliveryModal" role="dialog">
    <div class="modal-dialog modal-lg">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <input type="hidden" name="confirmBuyerId" id="confirmBuyerId" value=""/>
          <input type="hidden" name="confirmShipId" id="confirmShipId" value=""/>
...
function confirmDelivery(ordId,buyerId,shipId){
	$(".modal-footer #confirmOrdId").val( ordId );
	$(".modal-footer #confirmBuyerId").val( buyerId );
	$(".modal-footer #confirmShipId").val( shipId );
}

function confirmDeliveryById(){
	
	var ordId = $(".modal-footer #confirmOrdId").val();
	var buyerId = $(".modal-footer #confirmBuyerId").val();
	var shipId = $(".modal-footer #confirmShipId").val();
	...
	$.ajax({
		...
		success:function(data){
			console.log("賣家確認到貨");
			confirmDeliveryMessage("${memId}",buyerId,ordId,shipId)
		}
	})
