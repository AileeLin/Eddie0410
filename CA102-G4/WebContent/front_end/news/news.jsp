...
<%
	...
	List<NewsVO> newsVO =newsSvc.getAll();	
...
<head>
	...
	<!-- //隱藏iPhone Safari位址列的網頁 -->
	<!-- bootstrap-css -->
	<link href="<%=request.getContextPath()%>/front_end/css/news/news.css" rel="stylesheet" type="text/css" media="all" />
	<!-- //bootstrap-css -->
...
<body>
	...
	<div class="ui container">
		...
		<div class="container">
			<ul class="nav nav-tabs" id="myTab" role="tablist">
				<li class="nav-item active">
					<a class="nav-link active in" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-expanded="true">Travel Maker 報導</a>
				</li>
				<li class="nav-item">
					<a class="nav-link" id="profile-tab" data-toggle="tab" href="#profile" role="tab" aria-controls="profile" aria-expanded="false">天氣/匯率資訊</a>
				</li>
			</ul>
		...
		<%@ include file = "page1.file" %>
			...
			<c:forEach var="newsVO" items="${newsVO}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
					<div class="panel panel-success" >
						<div class="panel-heading"><c:out value="${newsVO.news_name}" />  <c:out value="${newsVO.news_date}" /></div>
						<div class="panel-body">							
							<c:out value="${newsVO.news_con}" />
						</div>
					</div>
			</c:forEach>
					...
					<nav aria-label="...">
						<ul class="pager">
							<li class="previous "><a href="<%=request.getRequestURI()%>?whichPage=<%=whichPage-1%>">Newer</a></li>
							<li class="next"><a href="<%=request.getRequestURI()%>?whichPage=<%=whichPage+1%>">Older<span aria-hidden="true"></span></a></li>
						</ul>
					</nav>
				</div>
				<div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
					<h3>旅行前先查看當地天氣</h3>
					<img src="<%=request.getContextPath()%>/front_end/images/news/map.jpg">
					點擊下方按鈕會將帶您查看最新天氣資訊
					<hr>
					<h1><a  style="margin-left: 800px;" class="label label-success" href="https://www.cwb.gov.tw/V7/forecast/world/world_aa.htm">Link</a></h1>
					<hr>
					<h3>旅行前先查看現今匯率</h3>
					<img src="<%=request.getContextPath()%>/front_end/images/news/rate.jpg">
					  點擊下方按鈕會將帶您查看最新匯率資訊
					<hr>
					<h1><a  style="margin-left: 800px;" class="label label-success" href="https://rate.bot.com.tw/xrt?Lang=zh-TW">Link</a></h1>
					<hr>
				</div>
			</div>
		</div>
