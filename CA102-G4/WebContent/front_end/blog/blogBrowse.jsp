...
<%
	...
	// 取得controller 回傳的list及其他資訊
	List<blogVO> list = (List)request.getAttribute("list");
	Set<blog_tagVO> blogTagList = (LinkedHashSet)request.getAttribute("blogTagList");
	Set<blog_tagVO> blogTagListHot = (LinkedHashSet)request.getAttribute("blogTagListHot");
	...
	request.setAttribute("blogTagList", blogTagList);
	request.setAttribute("blogTagListHot", blogTagListHot);
	...
	<div class="ui container">
		...
		<h1 class="ui header">
			...
			<p style="display: inline-block; font-size: 1rem">
				...
				<c:if test="${not empty blogTagList}">
				共 <span>${blogTagList.size()}</span> 篇
				</c:if>
				<c:if test="${not empty blogTagListHot}">
				共 <span>${blogTagListHot.size()}</span> 篇
				</c:if>
			</p>
		</h1>
		<!-- 我是隔板 -->
			...
			<div class="ui tabular menu">
				...
				<div class="ui massive right action left icon input">
					...
					<select class="basic floating dropdown button" name="item">
						...
						<div class="menu">
							<option class="item" value="content" <%= item.equals("content") ? "selected='selected'" : "" %>>標題、內容</option>
							<option class="item" value="tag" <%= item.equals("tag") ? "selected='selected'" : "" %>>標籤</option>
						</div>
					</select>
				</div>
				<!-- 搜尋BAR -->
		...
		<div class="ui four stackable cards">
			<c:forEach var="blogSvc" items="${blogSvc.allByNew}">
			<c:forEach var="blog_tagVO" items="${blogTagList}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
			<c:set var="bid" value="${blog_tagVO.blog_id}"/>
			<%
 				blogMessageService blogMessageSvc = new blogMessageService(); 
 				String bid = (String)pageContext.getAttribute("bid"); 
				List<blog_messageVO> blogMessageList = blogMessageSvc.findByBlogId(bid); 
 				pageContext.setAttribute("blogMessageList",blogMessageList); 
			%>
				<c:if test="${blogSvc.blog_id==blog_tagVO.blog_id}">
				<a class="fluid card" href="<%=request.getContextPath()%>/blog.do?action=article&blogID=${blogSvc.blog_id}">
					<div class="image">
						<div class="ui image pic">
							<img src="<%=request.getContextPath()%>/blogPicReader?blog_id=${blogSvc.blog_id}">
							<div class="ui teal ribbon label">遊記</div>
						</div>
					</div>
					<div class="content">
						<div class="header">${blogSvc.blog_title}</div>
						<div class="meta">
							<i class="far fa-calendar-alt"></i> 旅行日期:${blogSvc.travel_date}
						</div>
					</div>
					<div class="extra content">
						<span class="right floated">${blogMessageList.size()}個留言</span> <i class="far fa-eye"></i>
		...
		<div class="ui four stackable cards">
			<c:forEach var="blogSvc" items="${blogSvc.allByHot}">
			<c:forEach var="blog_tagVO" items="${blogTagListHot}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
			<c:set var="bid" value="${blog_tagVO.blog_id}"/>
			<%
 				blogMessageService blogMessageSvc = new blogMessageService(); 
 				String bid = (String)pageContext.getAttribute("bid"); 
