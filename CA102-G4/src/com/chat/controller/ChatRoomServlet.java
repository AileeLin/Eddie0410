...
public class ChatRoomServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if("addNewCR".equals(action)) {
			...
			if(input_newChatName == null || input_newChatName.trim().length() == 0) {
				errorMsgs.add("聊天對話名稱，請勿空白。");
			}
			...
			select_friList[select_friList.length] = myMemId; //把自己加入參加人員名單
			
			//先新增聊天對話及參與名單，拿到回傳的主鍵
			String pkey = crSvc.addChatRoom(input_newChatName,"",select_friList);
		...
		if("checkOnebyOneCR".equals(action)) {
			...
			try {
				...
				String pk = crSvc.addChatRoom(RoomName,"",addPeople);
			...
			try {
				...
				if(list.size() != 0) {
					...
					for(ChatRoom_JoinVO joinVO : list) {
						...
						tempStr+=("{\"ChatRoom_ID\":\""+crId+"\",\"memId\":\""+memId+"\",\"memName\":\""+memName+"\"},");
					}
